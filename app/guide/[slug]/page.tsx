...
export async function generateMetadata(props: GuidePostPageProps) {
  ...
  const post = getPublishedGuidePostBySlug(params.slug);
    ...
    <div className="min-h-screen flex flex-col bg-white font-pretendard">
      ...
      <main className="flex-1 container max-w-4xl mx-auto py-12 px-4 md:px-6">
        <article className="mx-auto">
          <ContentReadTracker contentType="guide" slug={post.slug} title={post.title} category={post.category} />
          {/* Back Link */}
          <div className="mb-8">
            <Link href="/guide">
          ...
          {enhancedFaq.length > 0 && (
            <div className="mt-20 pt-12 border-t-2 border-gray-100">
              ...
              <TrackedFaqAccordion items={enhancedFaq} slug={post.slug} contentType="guide" accent="green" />
            </div>
          )}
